...
public sealed class ComponentTaskCreateCommandHandler : CommandHandler<ComponentTaskCreateCommand>
{
    ...
    public override async Task<ICommandResult> HandleAsync(ComponentTaskCreateCommand command, IAsyncCollector<ICommand> commandQueue, IDurableOrchestrationContext orchestrationContext, ILogger log)
    {
        if (command is null)
