...
public sealed class ProjectUpdateCommandHandler : CommandHandler<ProjectUpdateCommand>
{
    ...
    public override async Task<ICommandResult> HandleAsync(ProjectUpdateCommand command, IAsyncCollector<ICommand> commandQueue, IDurableOrchestrationContext orchestrationContext, ILogger log)
    {
        if (command is null)
