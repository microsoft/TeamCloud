...
public sealed class ProjectDeployCommandHandler : CommandHandler<ProjectDeployCommand>
{
    ...
    public override async Task<ICommandResult> HandleAsync(ProjectDeployCommand command, IAsyncCollector<ICommand> commandQueue, IDurableOrchestrationContext orchestrationContext, ILogger log)
    {
        if (command is null)
