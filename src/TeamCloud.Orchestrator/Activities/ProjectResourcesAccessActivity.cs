...
//             var input = activitiyContext.GetInput<Input>();

//             var project = await projectRepository
//                 .GetAsync(input.OrganizationId, input.ProjectId)
//                 .ConfigureAwait(false);

//             if (project is null)
//                 throw new RetryCanceledException($"Could not find project '{input.ProjectId}'");
...
//                         resourceGroup.AddRoleAssignmentAsync(input.PrincipalId.ToString(), AzureRoleDefinition.Contributor),
//                         resourceGroup.AddRoleAssignmentAsync(input.PrincipalId.ToString(), AzureRoleDefinition.UserAccessAdministrator) // TODO: do we really need this ???
//                     };
