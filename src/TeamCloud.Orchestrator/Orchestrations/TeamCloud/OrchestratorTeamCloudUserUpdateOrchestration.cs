...
{
    public static class OrchestratorTeamCloudUserUpdateOrchestration
    {
        [FunctionName(nameof(OrchestratorTeamCloudUserUpdateOrchestration))]
            ...
            ILogger log)
        {
            ...
            var command = orchestratorCommand.Command as OrchestratorTeamCloudUserUpdateCommand;
            ...
            try
            {
                ...
                var user = await functionContext
                    .CallActivityWithRetryAsync<User>(nameof(TeamCloudUserUpdateActivity), command.Payload)
                    .ConfigureAwait(true);

                if (user.Role == UserRoles.TeamCloud.Admin)
                {
                    functionContext.SetCustomStatus("Waiting on project providers to update user.", log);
