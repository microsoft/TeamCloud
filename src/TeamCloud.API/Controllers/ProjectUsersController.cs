    ...
    public class ProjectUsersController : ApiController
    {
        ...
        public Task<IActionResult> Get() => ExecuteAsync(new Func<User, Organization, Project, Task<IActionResult>>(async (user, organization, project) =>
        {
            var users = await userRepository
                .ListAsync(organization.Id, project.Id)
                .ToListAsync()
        ...
        public Task<IActionResult> Get([FromRoute] string userId) => ExecuteAsync(new Func<User, Organization, Project, User, Task<IActionResult>>((contextUser, organization, project, user) =>
        {
        ...
        public Task<IActionResult> GetMe() => ExecuteAsync(new Func<User, Organization, Project, Task<IActionResult>>((contextUser, organization, project) =>
        {
        ...
        public Task<IActionResult> Post([FromBody] UserDefinition userDefinition) => ExecuteAsync(new Func<User, Organization, Project, Task<IActionResult>>(async (contextUser, organization, project) =>
        {
            ...
            var user = await UserService
                .ResolveUserAsync(organization.Id, userDefinition)
                .ConfigureAwait(false);
        ...
        [SuppressMessage("Usage", "CA1801: Review unused parameters", Justification = "Used by base class and makes signiture unique")]
        public Task<IActionResult> Put([FromRoute] string userId, [FromBody] User user) => ExecuteAsync(new Func<User, Organization, Project, User, Task<IActionResult>>(async (contextUser, organization, project, existingUser) =>
        {
            if (user is null)
            ...
            var command = new ProjectUserUpdateCommand(contextUser, existingUser, project.Id);
        ...
        [SwaggerResponse(StatusCodes.Status400BadRequest, "A validation error occured.", typeof(ErrorResult))]
        [SwaggerResponse(StatusCodes.Status404NotFound, "A Project with the provided projectId was not found, or a User matching the current user was not found.", typeof(ErrorResult))]
        public Task<IActionResult> PutMe([FromBody] User user) => ExecuteAsync(new Func<User, Organization, Project, Task<IActionResult>>(async (contextUser, organization, project) =>
        {
            if (user is null)
                throw new ArgumentNullException(nameof(user));
            ...
            if (!user.Id.Equals(contextUser.Id, StringComparison.Ordinal))
                return ErrorResult
            ...
            if (contextUser.IsOwner(project.Id) && !user.IsOwner(project.Id))
                return ErrorResult
                    .BadRequest($"Projects must have an owner. To change this user's role you must first transfer ownersip to another user.", ResultErrorCode.ValidationError)
                    .ToActionResult();
        ...
        [SwaggerResponse(StatusCodes.Status404NotFound, "A Project with the provided projectId was not found, or a User with the provided identifier was not found.", typeof(ErrorResult))]
        [SuppressMessage("Usage", "CA1801: Review unused parameters", Justification = "Used by base class and makes signiture unique")]
        public Task<IActionResult> Delete([FromRoute] string userId) => ExecuteAsync(new Func<User, Organization, Project, User, Task<IActionResult>>(async (contextUser, organization, project, user) =>
        {
            if (!user.IsMember(project.Id))
                return ErrorResult
